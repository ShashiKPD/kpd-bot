...
export const processPrivateMessage = async (client, message, chat) => {
  ...
  try{
    const userMessage = new UserMessage(message.id.id, message.from, message.to, message.mentionedIds, message.body, message.timestamp);
    // console.log(userMessage);
    const response = await getGptResponse(userMessage.content, systemPrompt, false);
